"""Analytical equations for D1-to-IL1, D1 > D2.
...
def validate_model_parameters():
    ...
    if D1 <= D2:
        raise ValueError(
            "The model uses the D1 > D2 region, so D1 must be greater than "
            "D2."
        )
...
def mutual_coupling_d1_greater_d2():
    """Return the mutual polynomial in the supplied D1 > D2 formulas."""
    return (
        D2 * RB2 * RB3
...
def solve_dc_operating_point():
    """Solve the two supplied D1 > D2 steady-state equations."""
    a11, a22 = diagonal_model_coefficients()
    mutual_coupling = mutual_coupling_d1_greater_d2()
...
def g_il1_d1(frequency_hz, IL1, IL2):
    """Evaluate the D1 > D2 form of I_L1_hat(s)/D1_hat(s)."""
