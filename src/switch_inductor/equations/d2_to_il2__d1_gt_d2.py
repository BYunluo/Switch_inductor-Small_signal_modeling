"""Analytical equations for D2-to-IL2, D1 > D2.
...
def diagonal_model_coefficients():
    """Return the two diagonal coefficients in the steady-state equations."""
...
def steady_state_mutual_coupling_d1_greater_d2():
    """Return the mutual polynomial shown in the supplied DC equations."""
    return (
        D2 * RB2 * RB3
        + (D1 - D2) * RB2 * (RB1 + RB3 + Ro)
        + (1.0 - D1) * RB1 * RB2
    )


def solve_dc_operating_point():
    """Solve the supplied D1 > D2 equations after setting both derivatives to zero."""
    a11, a22 = diagonal_model_coefficients()
    mutual_coupling = steady_state_mutual_coupling_d1_greater_d2()
...
def g_il2_d2(frequency_hz, IL1, IL2):
    """Evaluate I_L2_hat(s)/D2_hat(s) at s = j*2*pi*f."""
    ...
    denominator = (
        L1 * L2 * resistance_sum**2 * s**2
        + (
            L1 * resistance_sum * a22
            + L2 * resistance_sum * a11
        )
        * s
        + a11 * a22
        - mutual_coupling**2
